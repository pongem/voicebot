...
[docs]def read(file, frames=-1, start=0, stop=None, dtype='float64', always_2d=False,
...
[docs]def write(file, data, samplerate, subtype=None, endian=None, format=None,
          closefd=True):
...
[docs]def blocks(file, blocksize=None, overlap=0, frames=-1, start=0, stop=None,
...
[docs]def info(file, verbose=False):
...
[docs]def available_subtypes(format=None):
...
[docs]def check_format(format, subtype=None, endian=None):
...
[docs]def default_subtype(format):
...
[docs]    def seek(self, frames, whence=SEEK_SET):
...
[docs]    def read(self, frames=-1, dtype='float64', always_2d=False,
...
[docs]    def buffer_read(self, frames=-1, ctype='double'):
...
[docs]    def buffer_read_into(self, buffer, ctype='double'):
...
[docs]    def buffer_write(self, data, ctype):
...
[docs]    def blocks(self, blocksize=None, overlap=0, frames=-1, dtype='float64',
...
[docs]    def truncate(self, frames=None):
